...
export default function BioWriterScreen() {
  const { persona, tone, setTone, addGeneration, history, toggleFavorite, defaultToneBio } = useFlirtyfy()
  ...
  async function generate(overrideTone?: Tone) {
    ...
    setLoading(true)
    try {
      const generation = await generateDatingCopy({ kind: 'bio', input, tone: activeTone, persona })
  ...
  const remainingStyles = styles.filter(s => s !== tone)
  ...
  return (
    <ScreenShell title="Bio writer" subtitle="Write Tinder, Bumble, and Hinge bios that feel specific and premium." back>
      ...
      <View style={s.section}>
        <Text style={s.label}>Initial Vibe</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.chips}>
          {styles.map((item) => <Chip key={item} label={item} active={tone === item} onPress={() => setTone(item)} />)}
        </ScrollView>
      </View>

      <GradientButton label={loading ? 'Writing...' : results ? 'Rewrite set' : 'Rewrite my bio'} onPress={() => generate()} disabled={loading || input.trim().length < 8} />

      {loading && !results && (
        <View style={s.loading}><ActivityIndicator color={ACCENT} /><Text style={s.loadingText}>Polishing your bio to perfection...</Text></View>
      )}
        ...
        <View style={s.resultsContainer}>
          <View style={s.resultsHeader}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={s.label}>Bio Options ({tone})</Text>
